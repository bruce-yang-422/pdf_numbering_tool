...
├── numbering_tool.log         # 執行記錄檔（每次執行覆蓋）
├── coords.env                 # 座標＋參數設定檔
│
├── README.md
...
* **Log 記錄**：自動記錄執行過程到 `numbering_tool.log`（每次執行覆蓋）
...
5. 執行記錄會自動儲存到 `numbering_tool.log`
...
程式執行時會自動記錄到 `numbering_tool.log`：

* **固定檔名**：`numbering_tool.log`
